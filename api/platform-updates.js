...
//   • LIST_NAME must exactly match your "Platform Waitlist" list's name in ActiveCampaign
//     (this list is now dedicated to Platform Waitlist sign-ups only, separate from the
//     Sanctum Journal list, so each has its own independent unsubscribe).
...
// ---------- CONFIG ----------
const LIST_NAME = 'Platform Waitlist';   // <-- must match your ActiveCampaign list name exactly
...
module.exports = async (req, res) => {
  ...
  try {
    const { email = '', role = '', hpField = false, token = '' } = req.body || {};

    // 1) Honeypot: real people leave this unchecked. If it's checked, pretend success and stop.
    //    This is a checkbox (not a text field) specifically because browser autofill /
    //    password managers were found to auto-fill hidden TEXT fields with the user's own
    //    email address, causing false positives for real subscribers on the Journal form -
    //    fixed here too as a preventative measure, same root cause would apply.
    if (hpField) { console.log('Platform waitlist: honeypot checkbox was checked - treating as bot/false positive'); res.status(200).json({ ok: true }); return; }
