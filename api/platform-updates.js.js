...
// platform opens. It shares NO tags or lists with the Journal or the Enquiry form — someone
// signing up here is not added to the Journal, and vice versa.
//
...
//   4. Applies the "Platform Waitlist" tag — this is a BROAD marketing consent
...
//   5. If a role was selected (guest / host / venue owner), also applies a role-specific tag
//      so you can send more targeted launch content later if you want to.
//   6. Emails you an alert via Resend.
//
// TWO THINGS TO CONFIRM (see CONFIG below):
//   • TAG_NAME must exactly match (or will be created as) the tag name in ActiveCampaign.
//   • If you build an ActiveCampaign automation for a "you're on the waitlist" confirmation
//     email, trigger it off this tag being added.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 4) Apply the broad "Platform Waitlist" tag
    if (!cachedTagId) {
    ...
    // 5) If a role was picked, also apply that role-specific tag (optional, purely for
