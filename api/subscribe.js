...
module.exports = async (req, res) => {
  ...
  try {
    const { email = '', hpField = '', token = '', generalMarketing = false } = req.body || {};
    ...
    //    Logged here so it's visible in Vercel's function logs, since this path returns
    //    a normal-looking 200 response with no error - if this line never appears in the
    //    logs, the honeypot is NOT the cause of a missing submission.
    if (hpField) { console.log('Subscribe: honeypot triggered, value was:', hpField); res.status(200).json({ ok: true }); return; }
