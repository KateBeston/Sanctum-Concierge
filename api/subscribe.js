...
//   it must already exist under this exact name — "Sanctum Journal" — or the subscribe
//   step will be skipped, which would then block AC's automation from delivering the
//   welcome email, since that automation requires a real list subscription to fire).
...
module.exports = async (req, res) => {
  console.log('Subscribe: function invoked, method:', req.method);
  if (req.method !== 'POST') {
  ...
  try {
    const { email = '', hpField = false, token = '', generalMarketing = false } = req.body || {};
    console.log('Subscribe: received body for email:', email);

    // 1) Honeypot: real people leave this unchecked. If it's checked, pretend success and stop.
    //    This is now a checkbox (not a text field) specifically because browser autofill /
    //    password managers were found to auto-fill hidden TEXT fields with the user's own
    //    email address, causing false positives for real subscribers. Checkboxes are not
    //    targeted by autofill the same way, so this closes that loophole.
    if (hpField) { console.log('Subscribe: honeypot checkbox was checked - treating as bot/false positive'); res.status(200).json({ ok: true }); return; }
    ...
    }
    {
      ...
      if (!verifyData.success) {
        console.log('Subscribe: Turnstile verification FAILED', verifyData);
        res.status(400).json({ error: 'Verification failed. Please try again.' });
        return;
      }
      console.log('Subscribe: Turnstile verification passed');
    }
    ...
    console.log('Subscribe: AC contact sync result, contactId:', contactId, 'raw response:', JSON.stringify(syncData));
    ...
    console.log('Subscribe: Sanctum Journal tagId found/created:', tagId);
    if (contactId && tagId) {
      await fetch(base + '/api/3/contactTags', {
        method: 'POST', headers,
        body: JSON.stringify({ contactTag: { contact: contactId, tag: tagId } })
      });
      console.log('Subscribe: tag applied to contact', contactId);
    } else {
      console.log('Subscribe: SKIPPED tag application - missing contactId or tagId');
    }
